...
This layer includes all the information specific to each drone and the smartphone device. These are
\begin{itemize}
\item the orientation of the smartphone;
\item the location of the smartphone;
\item the information from the smartphone's camera;
\item the flight data for every drone;
\item the user preferences;
\item ...
\end{itemize}

\subsubsection{Drone flight data}
This is the most important part of the model, since it concerns critical data. The safety pilot makes decisions concerning the drones based on this data. 

The information concerning each drone are transmitted by the parser of the Networks layer. 

\subsection{Networks}
This layer includes a parser for converting messages received into data usable by the other layers for the GCS-to-drone and the drone-to-drone communications.
...
%The authors would also like to thank Jonas Gouraud, Juliette Rambourg, Julie Barbarin and Adrien Arnaud from ENAC for their help with using Paparazzi. 
